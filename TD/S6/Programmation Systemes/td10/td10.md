...
On créé un réseau docker comme ceci :
...
--network : permet de spécifier quel réseau docker sera utilisé.

Du côté du dockerfile de test on a ceci :
```Dockerfile
FROM ubuntu:22.04
...
CMD ["curl","node-app:8080"]
```

Ici le choix de l'image ubuntu:22.04 n'est pas forcément le meilleur mais c'était juste pour faire la vérification donc j'ai choisi une image déjà pull plus tôt.

Le container qui va run en lien avec ce dockerfile va également être run avec --network=mynetwork afin de pouvoir contacter le container node-app à partir de celui-ci.

Enfin lorsqu'on le build et qu'on le run lui aussi en parallèle du premier on obtient :
![[Pasted image 20230523134425.png | center]]
