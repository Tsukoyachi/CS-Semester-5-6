package ads.poo2.lab2.binaryTrees.ORIGIN;
...
public class BinaryTree<T> implements BinaryTreeInterface<T> {
	...
	public int lowness(BinaryTreeInterface<T> bt) {
		...
		return 1 + Math.min(lowness(bt.right()),lowness(bt.left()));
	}
	...
	private int leaves(BinaryTreeInterface<T> bt) {
		...
		return leaves(bt.right())+leaves(bt.left());
	}
	...
	private boolean balanced1(BinaryTreeInterface<T> bt) {
		...
		return (Math.abs(height(bt.left()) - height(bt.right())) <= 1) && balanced1(bt.left()) && balanced1(bt.right());
	}
	...
	private boolean shapely1(BinaryTreeInterface<T> bt) {
		...
		return (bt.height() <= 2*bt.lowness()) && shapely1(bt.left()) && shapely1(bt.right());
	} 
	...
	public boolean shapely2() {
		Pair res = shapely2(this);
		return res.height <= 2* res.lowness;
	}
